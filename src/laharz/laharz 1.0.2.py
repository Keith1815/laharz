# Version 1.0.2 Released: 14/11/21
...
def laharz():
    ...
    def CreateSurfaceMesh():
        ...
        dem_v_flip = np.flipud(dem_v) #not sure why, but is inverted left to right otherwise....
        ...
        for i in range(pmeshrescol):  # cols
            for j in range(pmeshresrow):  # rows
                ...
                #     h = h+1000
                v += [[x, y, h]]  # todo assumes square pixels

        log_msg("Preparing triangle faces...")
        f = []  # list of all horizontal lines

        for i in range(pmeshrescol-1):  # cols
            n = pmeshrescol #easier to read below
            for j in range(pmeshresrow-1):  # rows
                if p_diagonal == "SW-NE":
                    f += [[i*(n)+j, i*(n)+j+1, (i+1)*(n)+j+1]]
                    f += [[i*(n)+j, (i+1)*(n)+j+1, (i+1)*(n)+j]]
                else:
                    f += [[i*(n)+j, i*(n)+j+1, (i+1)*(n)+j]]
                    f += [[i*(n)+j+1, (i+1)*(n)+j+1, (i+1)*(n)+j]]
        ...
        # msh file for gmash
        mesh.export("../" + pwdir + "/" + pmesh_dir + "/Surface.ply")
        log_msg("Surface mesh written to: ../" + pwdir + "/" + pmesh_dir + "/Surface.ply")
    ...
    if pcalcinitpoints == 'calc':
        ...
        print("########################################################\n")
        print("Ignore the warning message below relating to division by zero\n")
        ...
        f_initpoints = open(pinitpoints_fn, "w", newline = '')
        ...
        if pplotmesh:
            ...
            maxr = np.amax(initrc[:, 1]) # maximum row of the initiation points
            minr = np.amin(initrc[:, 1]) # minimum row of the initiation points
            centr = int((maxr + minr) / 2) # mid row
            maxc = np.amax(initrc[:, 2]) # maximum column of the initiation points
            minc = np.amin(initrc[:, 2]) # minimum column of the initiation points
            centc = int((maxc + minc) / 2) # mid column
        ...
        if len(initrc)> 0:
            ...
            for v1, v in enumerate(pvolume):
                ...
                for i, ip in enumerate(initrc):
                    log_msg("Generating Lahar. Initiation Point: {}/{} Volume: {:.2e} {}/{} ".format(i + 1, initrc.shape[0], v, v1 + 1, len(pvolume)))
